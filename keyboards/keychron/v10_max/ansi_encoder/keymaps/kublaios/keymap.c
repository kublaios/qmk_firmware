...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_89(
        KC_MUTE,    KC_ESC,          KC_BRID,    KC_BRIU,  KC_F3,         KC_F4,         RGB_VAD,   RGB_VAI,  KC_MPRV,   KC_MPLY,       KC_MNXT,  KC_MUTE,  KC_VOLD,  KC_VOLU,  MC_8,               KC_DEL,
        ...
        KC_F23,     KC_LCTL,         KC_LOPTN,             KC_LCMMD,      LT(1,KC_SPC),  _______,                        LT(2,KC_SPC),            KC_ENT,                       KC_LEFT,  KC_DOWN,  KC_RGHT),
    ...
    if (record->event.pressed) {
        switch(keycode) {
            case M_DEV:
                ...
                return false;
            case M_DROPBOX:
                SEND_STRING(SS_DOWN(X_LCMD) SS_DOWN(X_LCTL) SS_DOWN(X_LALT) SS_TAP(X_4) SS_UP(X_LALT) SS_UP(X_LCTL) SS_UP(X_LCMD));
                return false;
            case M_HOME:
